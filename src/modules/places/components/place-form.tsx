  ...
  return (
    <div className="mx-auto w-full max-w-3xl space-y-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 rounded-3xl border border-border/80 bg-card p-6 shadow-xs sm:p-10">
        ...
        <section className="space-y-3"><label className="block text-sm font-medium">ภาพปก</label><div className="flex flex-col gap-4 sm:flex-row sm:items-center"><div className="flex h-40 w-full items-center justify-center overflow-hidden rounded-2xl border border-dashed border-border bg-muted/40 sm:w-64">{coverPreview ? <img src={coverPreview} alt="ตัวอย่างภาพปก" className="h-full w-full object-cover" /> : <ImagePlus className="h-8 w-8 text-muted-foreground/50" />}</div><div><label className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-border bg-card px-3.5 py-2 text-xs font-medium hover:bg-muted"><ImagePlus className="h-3.5 w-3.5" />{coverPreview ? "เปลี่ยนภาพปก" : "เลือกรูปภาพ"}<input type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={handleCoverChange} /></label><p className="mt-2 text-[11px] text-muted-foreground">PNG, JPG หรือ WebP ขนาดไม่เกิน 5 MB</p></div></div></section>
