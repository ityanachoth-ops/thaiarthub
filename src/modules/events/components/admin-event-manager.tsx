...
export function AdminEventManager({ events }: { events: AdminEventDetail[] }) {
  ...
  return (
    <section className="rounded-3xl border border-border/80 bg-card p-6 shadow-xs sm:p-8">
      ...
      {items.length === 0 ? <div className="mt-6 rounded-2xl border border-dashed border-border p-12 text-center text-sm text-muted-foreground"><Calendar className="mx-auto mb-2 h-6 w-6 text-primary" />ยังไม่มีกิจกรรม</div> : <div className="mt-6 divide-y divide-border/60">{items.map((event) => <div key={event.id} className="flex flex-col gap-3 py-5 first:pt-0 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-display font-semibold">{event.title}</h3><span className="rounded-full bg-muted px-2.5 py-1 text-[11px] text-muted-foreground">{event.status === "published" ? "เผยแพร่" : event.status === "cancelled" ? "ยกเลิก" : "ฉบับร่าง"}</span>{event.isFeatured ? <span className="rounded-full bg-primary/10 px-2.5 py-1 text-[11px] text-primary">เด่น</span> : null}{event.categories?.length ? (
        <span className="flex flex-wrap gap-1">
          {event.categories.map((cat) => (
            <span key={cat.id} className="rounded-full bg-blue-500/10 px-2 py-1 text-[10px] text-blue-700">
              {cat.name}
            </span>
          ))}
        </span>
      ) : null}</div><p className="mt-1 text-xs text-muted-foreground">{event.venueName ?? "ไม่ระบุสถานที่"} · /events/{event.slug}</p></div><div className="flex w-fit items-center gap-2"><Link href={`/dashboard/events/${event.id}/edit`} className="inline-flex items-center gap-1.5 rounded-lg border border-border px-3 py-2 text-xs font-medium"><Pencil className="h-3.5 w-3.5" />แก้ไขและจัดการแกลเลอรี</Link><button type="button" onClick={() => handleDelete(event)} disabled={isPending} className="inline-flex items-center gap-1.5 rounded-lg border border-destructive/20 px-3 py-2 text-xs font-medium text-destructive disabled:opacity-50"><Trash2 className="h-3.5 w-3.5" />{isPending && deletingId === event.id ? "กำลังลบ..." : "ลบ"}</button></div></div>)}</div>}
    </section>
  );
}
