  ...
  return (
    <div className="flex min-h-screen flex-col bg-background text-foreground selection:bg-primary/15 selection:text-primary">
      <header className="sticky top-0 z-40 border-b border-border/70 bg-background/90 backdrop-blur-md">
        ...
        <div className="mx-auto hidden min-h-16 max-w-6xl items-center px-4 sm:px-6 md:grid md:grid-cols-[1fr_auto_1fr]">
          {/* Left nav */}
        ...
        <div className="mx-auto flex min-h-14 max-w-6xl items-center justify-between gap-3 px-4 md:hidden">
          <Link
            href="/"
          ...
          <div className="flex items-center gap-2">
            <Link
              href="/search"
              className="flex items-center gap-1.5 rounded-full border border-border bg-card px-3 py-1.5 text-xs font-medium text-muted-foreground transition hover:border-primary/40 hover:text-primary"
              aria-label="ค้นหา"
            >
              <Search className="h-3.5 w-3.5 text-primary" />
              <span>ค้นหา</span>
        ...
        <div className="border-t border-border/40 md:hidden">
          <nav
            aria-label="เมนูหลัก"
            className="mx-auto flex max-w-6xl items-center gap-1 overflow-x-auto px-4 py-2 text-sm"
          >
            {navigation.map((item) => (
              <NavLink key={item.href} href={item.href} label={item.label} pathname={pathname} />
            ))}
