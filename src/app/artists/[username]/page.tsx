...
export default async function ArtistProfilePage({
  ...
  return (
    <div className="flex flex-col gap-10">
        ...
        <div className="flex flex-col gap-5 p-6 sm:p-8">
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-micro text-muted-foreground">@{artist.slug}</p>
              {artist.location ? (
          ...
          {artist.bio ? (
            <p className="max-w-3xl whitespace-pre-line text-body text-muted-foreground">
              {artist.bio}
            </p>
          ) : null}
            ...
            <div className="flex flex-wrap gap-2.5">
              {externalLinks.map(([label, href]) => (
                <a
                  ...
                  className="inline-flex items-center gap-1.5 rounded-md border border-border bg-background px-3.5 py-2 text-sm font-medium text-foreground transition hover:border-primary/40 hover:text-primary"
                >
                  {label}
      ...
      {artist.gallery && artist.gallery.length > 0 ? (
        <section className="space-y-4">
          <div className="flex items-baseline justify-between gap-4 border-b border-border/50 pb-2">
            ...
            <span className="text-micro text-muted-foreground">{artist.gallery.length} รูป</span>
          </div>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-5">
            {artist.gallery.map((image) => (
              <div key={image.id} className="aspect-square overflow-hidden rounded-lg border border-border bg-muted/30">
                {image.imageUrl ? (
      ...
      <section className="space-y-4">
        <div className="flex items-baseline justify-between gap-4 border-b border-border/50 pb-2">
          <h2 className="text-display-sm text-foreground">ผลงาน</h2>
          <span className="text-micro text-muted-foreground">{artworks.length} ชิ้น</span>
        </div>
        {artworks.length > 0 ? (
      ...
      <section className="space-y-4 section-space-lg">
        <div className="flex items-baseline justify-between gap-4 border-b border-border/50 pb-2">
          <h2 className="text-display-sm text-foreground">กิจกรรมที่เกี่ยวข้อง</h2>
          <span className="text-micro text-muted-foreground">{events.length} กิจกรรม</span>
        </div>
